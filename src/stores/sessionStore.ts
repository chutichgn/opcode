...
> = (set, get) => ({
      ...
      set({
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch session output",
        isLoadingOutputs: false,
      });
    }
  },
  ...
  deleteSession: async (sessionId: string, projectId: string) => {
    try {
      // Delete session files from disk via API
      ...
      set((state) => ({
        sessions: {
          ...state.sessions,
          [projectId]:
            state.sessions[projectId]?.filter((s) => s.id !== sessionId) || [],
        },
        currentSessionId:
        ...
        sessionOutputs: Object.fromEntries(
          Object.entries(state.sessionOutputs).filter(
            ([id]) => id !== sessionId,
          ),
        ),
      }));
  ...
  handleSessionUpdate: (session: Session) => {
    set((state) => {
      ...
      const existingIndex = projectSessions.findIndex(
        (s) => s.id === session.id,
      );
