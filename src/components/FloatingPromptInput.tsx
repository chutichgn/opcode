...
try {
  ...
  console.log(
    "[FloatingPromptInput] Tauri webview API not available, using web mode",
  );
}
...
interface FloatingPromptInputProps {
  ...
  /**
   * Callback when /kill action command is selected to delete the current session
   */
  onKillSession?: () => void;
}
...
const FloatingPromptInputInner = (
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenDragDropRef.current = await webview.onDragDropEvent(
          (event: any) => {
            if (
  ...
  const handleSlashCommandSelect = (command: SlashCommand) => {
    // Handle action commands that execute logic instead of inserting text
    if (
      command.id === "default-kill" ||
      command.id === "default-session-kill"
    ) {
      setShowSlashCommandPicker(false);
      setSlashCommandQuery("");
      setPrompt("");
  ...
  const isIMEInteraction = (event?: React.KeyboardEvent) => {
    ...
    const keyCode =
      ...
      (keyboardEvent as unknown as { which?: number }).which;
    if (keyCode === 229) {
      return true;
    }
                    ...
                    >
                      ...
                      >
                        <Button
                          ...
                          disabled={
                            isLoading ? false : !prompt.trim() || disabled
                          }
                          variant={
                            isLoading
                              ? "destructive"
