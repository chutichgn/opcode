...
const listen =
  ...
  ((eventName: string, callback: (event: any) => void) => {
    console.log(
      "[ClaudeCodeSession] Setting up DOM event listener for:",
      eventName,
    );
    ...
    const domEventHandler = (event: any) => {
      console.log(
        "[ClaudeCodeSession] DOM event received:",
        eventName,
        event.detail,
      );
    ...
    return Promise.resolve(() => {
      console.log(
        "[ClaudeCodeSession] Removing DOM event listener for:",
        eventName,
      );
...
interface ClaudeCodeSessionProps {
  ...
  /**
   * Callback when session is deleted via /kill or /session_kill
   */
...
export const ClaudeCodeSession: React.FC<ClaudeCodeSessionProps> = ({
      ...
      if (!isListeningRef.current) {
        ...
        function handleStreamMessage(payload: string | ClaudeStreamMessage) {
          try {
            ...
            console.log(
              "[ClaudeCodeSession] handleStreamMessage - message type:",
              message.type,
            );
            ...
            if (message.type === "assistant" && message.message?.content) {
              ...
              if (codeBlocks.length > 0) {
                ...
                codeBlocks.forEach((block: any) => {
                  ...
                  sessionMetrics.current.codeBlocksGenerated += Math.floor(
                    matches / 2,
                  );
                });
              }
  ...
  /**
   * Handles confirmed session deletion via /kill or /session_kill.
   * Cancels any running execution, deletes session files from disk,
   * clears local store state, and notifies parent to close the tab.
   */
  const handleConfirmKill = async () => {
    const sessionId = claudeSessionId || effectiveSession?.id;
    const projectId =
      effectiveSession?.project_id || extractedSessionInfo?.projectId;
    ...
    try {
      // Cancel execution if currently running
      if (isLoading && claudeSessionId) {
        try {
          await api.cancelClaudeExecution(claudeSessionId);
        } catch (err) {
          console.warn("Failed to cancel execution during kill:", err);
        }
        unlistenRefs.current.forEach((unlisten) => unlisten());
        unlistenRefs.current = [];
        setIsLoading(false);
          ...
          <ErrorBoundary>
            ...
            >
              <FloatingPromptInput
                ...
                onKillSession={() => setShowKillDialog(true)}
                isLoading={isLoading}
        ...
        <Dialog open={showKillDialog} onOpenChange={setShowKillDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                This will permanently delete the session files from disk and
                close this tab. This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
